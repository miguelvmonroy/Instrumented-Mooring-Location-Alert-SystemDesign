    ...
    try:
        if "+EVT:RXP2P" in response:
            ...
            if len(parts) >= 4:
                ...
                print(f"Datos decodificados (texto): {text}")
                return text
...
if __name__ == "__main__":
    ...
    send_at_command("AT+P2P=915000000,7,125,1,14,8")  # Configuración LoRa P2P

    print("Esperando mensajes del transmisor...")
    send_at_command("AT+PRECV=65535")  # Activar recepción continua

    with serial.Serial(configRX.PORT, configRX.BAUDRATE, timeout=5) as ser:
        while True:
            if ser.in_waiting > 0:  # Verifica si hay datos disponibles
                response = ser.read_all().decode(errors='ignore')
                process_rx_event(response)  # Procesa y muestra el mensaje recibido
